    ...
    with open(os.path.join(CHECKPOINT_DIR, cp_file), 'r') as f:
        ...
        if match:
            ...
            for filepath in items:
                filepath = filepath.strip('`').split()[0] # get first word
        ...
        if match2:
            ...
            for filepath in items:
                ...
                if not os.path.exists(os.path.join(CODEBASE_ROOT, filepath)) and filepath not in created_files:
                    ...
                    if not filepath.endswith('.py') and not filepath.endswith('.md'): continue
...
# Run a dummy git check to see if drift occurred
try:
    current_commit = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=CODEBASE_ROOT).decode('utf-8').strip()
    status = subprocess.check_output(['git', 'status', '--porcelain'], cwd=CODEBASE_ROOT).decode('utf-8').strip()
    if status:
        # Codebase has changed!
        pass # Not blocking for audit unless specific
except Exception:
