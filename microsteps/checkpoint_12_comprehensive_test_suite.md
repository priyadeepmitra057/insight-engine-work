...
class TestConfigPassionCanonicals:
  ...
  [ ] grep -n "Detection Engine — Master Plan" tests/test_passion_engine.py returns no matches.
  [ ] grep -n "^## " tests/test_passion_engine.py returns no matches.
  [ ] grep -n "^### " tests/test_passion_engine.py returns no matches.
  [ ] grep -n "^import pytest" tests/test_passion_engine.py returns exactly one match.
...
code = Path("tests/test_passion_engine.py").read_text(encoding="utf-8")
ast.parse(code)

assert "Detection Engine — Master Plan" not in code
assert "\n## " not in code
assert "\n### " not in code
assert "import pytest" in code
assert code.count("BEGIN_CB_P7_05") == 1
assert code.count("END_CB_P7_05") == 1
assert code.count("BEGIN_CB_P8_01") == 1
assert code.count("END_CB_P8_01") == 1
assert len(re.findall(r"^\s*def test_", code, re.M)) >= 100

print("test_passion_engine.py embedded suite validation passed")
PY
