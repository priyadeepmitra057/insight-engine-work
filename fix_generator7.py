...
for cp_file in cp_files:
    with open(os.path.join(CHECKPOINT_DIR, cp_file), 'r') as f:
        content = f.read()
        created_files = []
        for match in re.finditer(r'File:\s*(.*?)\n\s*Action:\s*(CREATE|CREATE_OR_MODIFY|CONDITIONAL_MODIFY)', content, re.IGNORECASE | re.MULTILINE):
...
# We should also check CODEBASE DRIFT correctly for the Before blocks
# Actually doing drift properly: for every Before block, is it in the target file?
import difflib

# Dummy drift check for now
# We will just write a few mock ones to see if there is any other issue we missed.
# Let's actually check context collision on before block in config.py
try:
    with open(os.path.join(CODEBASE_ROOT, 'config.py'), 'r') as f:
        config_content = f.read()
except:
    config_content = ""

# Instead of perfectly matching everything, let's look for missing before blocks
for cp_file in cp_files:
    with open(os.path.join(CHECKPOINT_DIR, cp_file), 'r') as f:
        content = f.read()
        ...
        for i, step_tuple in enumerate(steps):
            step = step_tuple[0]
            if 'Action:         INSERT' in step or 'Action:         REPLACE' in step:
                # Find Target File
                ...
                if before_match:
                    before_code = before_match.group(1)
                    # Check if before_code is exactly in target file
                    try:
                        with open(os.path.join(CODEBASE_ROOT, target_file), 'r') as tf:
                            tf_content = tf.read()
                            if before_code not in tf_content:
                                add_blocker("BEFORE BLOCK NOT FOUND", "L2→L3", cp_file, f"{target_file} Step {i+1}",
                                            f"Before block for step {i+1} does not match exactly in {target_file}",
                                            "Before block not found in file",
                                            "Update the Before block to match the file or update the file")
