...
# Codebase Drift Detection properly
# We check git if files modified
try:
    current_commit = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=CODEBASE_ROOT).decode('utf-8').strip()
    # Find drift by comparing with the time the checkpoint was written. But we don't have checkpoint_written_commit!
    # Just check if files are drifting compared to Before blocks
    for cp_file in cp_files:
        with open(os.path.join(CHECKPOINT_DIR, cp_file), 'r') as f:
            content = f.read()
