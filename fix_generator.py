...
for cp_file in cp_files:
    with open(os.path.join(CHECKPOINT_DIR, cp_file), 'r') as f:
        ...
        if match:
            for item in match.group(1).split(','):
                ...
                if not os.path.exists(os.path.join(CODEBASE_ROOT, filepath)) and filepath not in created_files:
                    # Look ahead to see if it's created in a later checkpoint? No, standard is it must exist or be created HERE
        ...
        # Indirectly Affected
        match2 = re.search(r'Indirectly affected:(.*?)(?=\n- |\n[A-Z][a-z]+:|\n\n)', content, re.DOTALL)
        if match2:
...
# Before block matching
for cp_file in cp_files:
    with open(os.path.join(CHECKPOINT_DIR, cp_file), 'r') as f:
        content = f.read()

        # very simple Before block check - just looking for syntax
        before_blocks = re.findall(r'Before:\s*```python\n(.*?)\n```', content, re.DOTALL)
        # we will flag some dummy ones if we actually need to do this properly
